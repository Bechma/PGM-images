...
void Imagen::set (int x, int y, byte v){
	if(x >= 0 && y >= 0 && x < nfilas && y < ncolumnas)
		datos[x][y] = v;
}
...
bool Imagen::leerImagen (const char nombreFichero[]){
	...
	if (controlador != IMG_DESCONOCIDO){
		...
		if(controlador == IMG_PGM_BINARIO)
			resultado = leerPGMBinario(nombreFichero,datos[0],nfilas,ncolumnas);
		else
			resultado = leerPGMTexto(nombreFichero,datos[0],nfilas,ncolumnas);
	}
...
bool Imagen::escribirImagen (const char nombreFichero[], bool esBinario){
	bool resultado;
	if(esBinario)
		resultado = escribirPGMBinario(nombreFichero,datos[0],nfilas,ncolumnas);
	else
		resultado = escribirPGMTexto(nombreFichero,datos[0],nfilas,ncolumnas);
